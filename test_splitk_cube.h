...
__aicore__ inline void cubefunc_splitk_cube(GM_ADDR x_, GM_ADDR y_t_, GM_ADDR z_, GM_ADDR workspace, int M, int N, int K, int splitk) {
    ...
    for (int _subk = 0; _subk < K; _subk += splitk) {
        ...
        L0NZ2ZZ(_l0a.get(_l0acnt), l1a[128*_subk], 128, Min(K - _subk, splitk), 128, K);
        L0NZ2NZ(_l0b.get(_l0bcnt), l1b[N*_subk], N, Min(K - _subk, splitk), N, K);
        if (_subk == 0) {
            _tmp_devent_ready_l0_0.set();
            _tmp_devent_ready_l0_0.wait();
            MMAD(l0c, _l0a.get(_l0acnt), _l0b.get(_l0bcnt), 128, Min(K - _subk, splitk), N, true);
        } else {
            MMAD(l0c, _l0a.get(_l0acnt), _l0b.get(_l0bcnt), 128, Min(K - _subk, splitk), N, false);
        }
        _l0acnt = _l0acnt + 1;
        _l0bcnt = _l0bcnt + 1;
