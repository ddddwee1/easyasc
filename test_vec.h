...
__aicore__ inline void cubefunc_vec(GM_ADDR x_, GM_ADDR y_, GM_ADDR z_, GM_ADDR workspace, int M, int N, int K) {
    ...
    for (int m = m1; m < m2; m += 128) {
        ...
        GM2UBPAD(xubs, y[m * N + 0], (m + 128) - m, (K - 0) * 2, (K - ((m + 128) - m)) * 2, CeilDiv(K - (K - 0), 16));
        VEC_READY(0, PIPE_MTE2);
        ...
        for (int i = 0; i < 10; i += 1) {
            ...
            GM2UBPAD(xub.get(cnt), x[m * K + 0], (m + 128) - m, (K - 0) * 2, (M - ((m + 128) - m)) * 2, CeilDiv(K - (K - 0), 16));
        ...
        UB2GMPAD(vv[m * N + 0], xub.get(cnt), (m + 128) - m, (K - 0) * 2, CeilDiv(K - (K - 0), 16), (M - ((m + 128) - m)) * 2);
