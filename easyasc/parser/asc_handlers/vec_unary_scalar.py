...
def _handle_vec_unary_scalar(inst, helper, expr_map, func_name: str) -> None:
    ...
    if not isinstance(src, Tensor):
        raise TypeError(f"{func_name}requires Tensor type, current type: {type(src)}")
...
def handle_vec_axpy(inst, helper, expr_map) -> None:
    dst = inst.kwargs.get("dst", None)
    if not isinstance(dst, Tensor):
        raise TypeError(f"Axpy requires Tensor type, current type: {type(dst)}")
    src = inst.kwargs.get("src", None)
    if not isinstance(src, Tensor):
        raise TypeError(f"Axpy requires Tensor type, current type: {type(src)}")
